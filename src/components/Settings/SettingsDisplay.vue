...
export default {
  ...
  methods: {
    ...
    setVoice(){
      ...
      this.speech.voice = this.voices[document.querySelector("#voiceSelect").value];
...
<template>
  <div class="container">
    ...
    <form class="settings-container" >
      ...
      <select id="voiceSelect" class="form-select" v-model="selectedVoice" @change="setVoice()" ></select>
      <br/>
      <label class="form-label">Type here to hear the voice selected</label>
      <textarea rows="3" class="form-control" placeholder="Type here to hear the voice selected"></textarea>
      <div class="btn-group">
        <button id="play" class="btn btn-primary mt-5 me-3" @click="play">Speak</button>
        <button id="pause" class="btn btn-primary mt-5 me-3" @click="pause">Pause</button>
        <button id="stop" class="btn btn-primary mt-5 me-3" @click="cancel">Stop</button>
      </div>

      <input type="range" min="0" max="1" step="0.1" id="volume" @input="setVolume()" class="slider"/>
      <span id="volume-label" class="ms-2">1</span>
      <label class="form-label">Volume</label>
      
      <input type="range" min="0" max="1" value="1" step="0.5" id="rate" @input="setRate()" class="slider"/>
      <span id="rate-label" class="ms-2">1</span>
      <label class="form-label">Rate</label>

      <input type="range" min="0" max="1" value="1" step="0.5" id="pitch" @input="setVolume()" class="slider"/>
